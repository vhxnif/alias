...
import { oraText, printCmdLog, printErr, printTable, selectRun, tableConfig, tableDefaultConfig, title } from "../utils/common-utils"
...
export default class GitAlias implements IGitAlias {
    ...
    log = async (limit: number, author?: string) => {
        ...
        const data = this.lines(logs).map(it => it.split('│').map((it, idx) => ds[idx]?.(it)))
        printTable([title(['Hash', 'Author', 'Message', 'Date']), ...data], tableConfig([1, 1, 3.5, 2.5]))
    }
    ...
    status = async () => {
        ...
        const data = statusShortLog.map(it => it.map((item, idx) => ds[idx](item)))
        printTable([title(['Stash', 'Change', 'File']), ...data], tableConfig([1, 1, 3]))
    }
    push = async () => { await this.exec('git push') }
    ...
    branchList = async (name?: string, isListAll?: boolean) => {
        await this.exec(`git branch ${isListAll ? '-a' : '-l'}`)
            ...
            .then(names => names.map(it => (it.startsWith("*") ? [color.yellow(it.replace("*", "").trim())] : [color.blue(it.trim())])))
    ...
    branchCheckout = async (name?: string) => await this.branchAction(s => `git switch ${s}`, name)
    branchMerge = async (name?: string) => await this.branchAction(s => `git merge ${s}`, name)
    branchRebase = async (name?: string) => await this.branchAction(s => `git rebase ${s}`, name)
    branchDelete = async (name?: string) => await this.branchAction(s => `git branch -D ${s}`, name)
    ...
    stashList = async () => {
        ...
        printTable([title(['StashNo', 'Message', 'Autore', 'Date']), ...data], tableConfig([1, 3, 1, 1]))
    }

    stashAdd = async (name: string) => await this.exec(`git stash push -m ${name}`).then(() => this.stashList())
    stashPop = async () => await this.exec(`git stash pop`).then(this.status)
    stashShow = async () => await this.stashAction(async s => {
        await this.exec(`git stash show -p ${s}`).then(printCmdLog)
    })
    stashApply = async () => await this.stashAction(async s => {
        await this.exec(`git stash apply ${s}`).then(printCmdLog)
    })
    stashDrop = async () => await this.stashAction(async s => {
        await this.exec(`git stash drop ${s}`).then(printCmdLog)
    })
    add = async () => await this.fileProcess('Select Add Files:', `git add`, this.changedFile)
    restore = async () => await this.fileProcess('Select Restore Files:', 'git restore --staged', this.stagedFile)
    commit = async () => {
        ...
        await checkbox({
            ...
            await this.exec(`git add ${answer.join(' ')}`)
                .then(this.commitWithMessage)
        })
    }
    ...
    private statusLogFilter = async (f: (str: string[]) => boolean) => await this.statusShortLogInfo('git status -sunormal').then(it => it.filter(f))
    private changedFile = async () => await this.statusLogFilter((str: string[]) => str[1] !== " ")
    private stagedFile = async () => await this.statusLogFilter((str: string[]) => ![" ", "?"].includes(str[0]))

    private fileProcess = async (message: string, commandPre: string, logs: () => Promise<string[][]>, singleRun: boolean = true) => {
    ...
    private statusShortLogInfo = async (command: string) => {
        const lines = await this.exec(command).then(it => it.split('\n')).then(it => it.filter(l => !isEmpty(l)))
        if (isEmpty(lines)) {
            return []
        }
        return lines.map(it => [
    ...
    private branchAction = async (cf: (str: string) => string, name?: string, opt?: string, cuf?: (branchName: string) => boolean) => {
        const choices = await this.exec(`git branch ${opt ? opt : '-l'}`)
            .then(logs => this.branchNames(logs, true, name).filter(it => cuf ? cuf(it) : true))
            .then(names => names.map(it => ({ name: it, value: it })))
        if (isEmpty(choices)) {
            printErr('Branch Not Match.')
            return
        }
        selectRun('Select Branch:', choices, async s => await this.execPrint(cf(s)))
    }

    private execPrint = async (command: string) => printCmdLog(await this.exec(command))
    ...
    private lines = (str: string) => str.trim().split("\n").map(it => it.trim())
    private branchNames = (logs: string, exCurr: boolean, nameFilter?: string) => {
